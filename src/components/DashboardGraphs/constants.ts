
export const COLORS = {
  positive: '#10b981',  // Brighter green for better contrast
  neutral: '#6b7280',   // Slightly lighter gray
  negative: '#ef4444',  // Brighter red
  overview: ['#10b981', '#6b7280', '#ef4444'],
  aspects: ['#8B5CF6', '#EC4899', '#F59E0B', '#10B981', '#3B82F6'],
  wordcloud: {
    positive: '#22c55e',  // Brighter positive green
    neutral: '#9ca3af',   // Lighter neutral gray
    negative: '#f87171'   // Brighter negative red
  },
  sources: ['#3B82F6', '#8B5CF6', '#EC4899', '#F59E0B', '#10B981'],
  mentioned: ['#67e8f9', '#818cf8', '#a78bfa', '#c084fc', '#e879f9', '#f472b6', '#fb7185'],
  confusionMatrix: ['#fecdd3', '#fda4af', '#fb7185', '#f43f5e', '#e11d48', '#be123c', '#881337']
};
